...
- [环境变量](#环境变量)
- [v1.0.4 beta1 更新说明](#v104-beta1-更新说明)
- [配置说明](#配置说明)
- [管理员配置](#管理员配置)
...
```bash
# 拉取预构建镜像
docker pull ghcr.io/lunatechlab/wenxitv:latest

# 运行容器
# -d: 后台运行  -p: 映射端口 3000 -> 3000
docker run -d --name wenxitv -p 3000:3000 --env PASSWORD=your_password ghcr.io/lunatechlab/wenxitv:latest
```
...
services:
  wenxitv-core:
    image: ghcr.io/lunatechlab/wenxitv:latest
    container_name: wenxitv-core
...
| NEXT_PUBLIC_SITE_NAME               | 站点名称                                     | 任意字符串                       | WenXiTV                                                                                                                    |
...
| NEXT_PUBLIC_ENABLE_AI_4K            | 默认启用 AI4K 画质增强                        | true/false                       | false                                                                                                                      |
| NEXT_PUBLIC_AI_ENHANCE_WIDTH        | 自动开启增强的宽度阈值（像素）                | 整数                             | 1280                                                                                                                       |
| NEXT_PUBLIC_AI_4K_FORCE_DISABLE     | 强制禁用 AI4K 并隐藏相关控件                  | true/false                       | false                                                                                                                      |

快速配置：你可以复制根目录新增的 `.env.example` 为 `.env.local` 或部署平台的环境变量，并按需修改。

更多变量与默认行为请参考 `.env.example` 文件中的注释。

## v1.0.4 beta1 更新说明

本版为功能增强与体验优化的 Beta 版本，重点如下：

- 新增「回到顶部」桌面悬浮按钮：在桌面端主内容区右下角显示，点击即可平滑滚动到页面顶部。该按钮自动绑定主滚动容器，无需额外配置。
- 内容横向滚动增强：支持使用鼠标滚轮（垂直滚动）自动转换为横向滚动，浏览横排内容更自然顺手。
- 液态玻璃效果优化：增强透明度、边框与内阴影，并优化高光位置与大小，DesktopBottomNav 等组件在亮暗色模式下均更通透。

使用说明与建议：

- 桌面端建议在宽屏下体验，悬浮按钮默认启用，无需配置。
- 横向滚动在包含横向内容的行组件中生效，不影响普通竖向滚动页面。
- 如需自定义站点名称（影响网页标题与部分文案），请设置 `NEXT_PUBLIC_SITE_NAME` 环境变量，例如：`NEXT_PUBLIC_SITE_NAME=MoonTV`。

以上变更已同步到 `CHANGELOG` 与 `src/lib/changelog.ts`，版本面板会展示当前版本信息与更新状态。
...
WenXiTV 支持标准的苹果 CMS V10 API 格式。
...
[MIT](LICENSE) © 2025 WenXiTV & Contributors
